...
(** Scan log file with pattern matching.
    ...
    based on pattern match_type. Returns Success/Error/None. *)
let scan : type a ctx.
       ?logfile:string
    -> ?logger:(string -> unit)
    -> ?start_pos:int
    -> (a, ctx) LogFile.pattern_db
    -> ([> `Success of a | `Not_found], stunnel_error) result =
 fun ?(logfile = "") ?(logger = fun _ -> ()) ?(start_pos = 0) patterns ->
  ...
  let scan_result =
    LogFile.stream_from_position logfile start_pos process_line ([], None)
  in
  ...
  | Ok ((context, Some pattern), _) -> (
    ...
    | LogFile.Success x ->
        Ok (`Success x)
    | LogFile.Error x ->
        (* For string patterns with cert context, convert error *)
        let err = Obj.magic x in
        if List.length context > 0 then
          (* Context is accumulated cert errors (string list) *)
          Error (Patterns.to_stunnel_error (Obj.magic context) err)
        else
          Error (Unknown err)
  )
  | Ok ((_, None), _) ->
...
let check_errors_from_position ~logfile ?(logger = fun _ -> ()) start_pos =
  let error_patterns =
    Patterns.
      [
  ...
  in
  match scan ~logfile ~logger ~start_pos error_patterns with
  | Ok (`Success _) ->
      Ok ()
  | Ok `Not_found ->
      Ok ()
...
let check_errors ~logfile ?(logger = fun _ -> ()) () =
  check_errors_from_position ~logfile ~logger 0
