...
module LogFile : sig
  ...
  type 'a pattern_match = Success of 'a | Error of 'a
...
(** Scan log file with pattern matching.
    Automatically handles certificate error context accumulation for patterns
    with AccumulateUntilMatch type. *)

val check_errors_from_position :
     logfile:string
  -> ?logger:(string -> unit)
  -> int
