...
let wait_for ~delay ~max_retries ~timeout_error ~extra_condition ~logfile
    patterns =
  let rec check cnt =
    Thread.delay delay ;
    match (extra_condition (), Stunnel_log.scan ~logfile patterns) with
    | true, Ok (`Success _) ->
        Ok ()
    | _, Error err ->
        Error err
    | _, _ when cnt > max_retries ->
        Error (Stunnel timeout_error)
    | _, _ ->
        check (cnt + 1)
  in
  check 0
...
let wait_for_init_done unix_socket_path logfile =
  ...
  wait_for ~delay:1.0 ~max_retries:3
    ~timeout_error:"Timed out when initialising stunnel"
    ~extra_condition:(fun () -> Sys.file_exists unix_socket_path)
    ~logfile patterns
...
let wait_for_connection_done logfile =
  ...
  in
  wait_for ~delay:0.5 ~max_retries:10
    ~timeout_error:"Timed out waiting for connection attempt"
    ~extra_condition:(fun () -> true)
    ~logfile patterns
  ...
  let diagnose handle =
    ...
    if current_size <= start_pos then (
      ...
      let finally = Xapi_stdext_pervasives.Pervasiveext.finally in
        ...
        (fun () -> Unix.close fd) ;
      match
        Stunnel_log.check_errors_from_position ~logfile:handle.proxy_logfile
          start_pos
      with
      | Ok () ->
